...
tree.genauigkeit <- matrix(0,nrow=10,ncol=10)
for(i in 1:10){
...
randomForest.genauigkeit <- matrix(0,nrow=10,ncol=10)
for(i in 1:10){
...
set.seed(1)
time.start <- Sys.time()
boosting.MNIST <- gbm(label~.,data=data.train[training,],distribution="multinomial",n.trees=500,interaction.depth=32,shrinkage=0.5,n.cores=4)
time.end <- Sys.time()
boosting.duration <- time.end - time.start # Messung der Zeitdauer der Berechnung
boosting.pred <- predict(boosting.MNIST,newdata=data.train[-training,],n.trees=500) # Einordnung der Testdaten anhand des Modells
boosting.pred.multi <- apply(boosting.pred, 1, which.max) # Zuordnen der labels
boosting.tabelle <- table(pred=boosting.pred.multi,true=data.train[-training,1]) # Und folgende: Berechnung der Testgenauigkeit
boosting.genauigkeit <- matrix(0,nrow=10,ncol=10)
for(i in 1:10){
...
bagging.genauigkeit <- matrix(0,nrow=10,ncol=10)
for(i in 1:10){
...
tree.genauigkeit.pure <- matrix(0,nrow=10,ncol=10)
for(i in 1:10){
...
set.seed(1)
time.start <- Sys.time()
rpart.MNIST.pure <- rpart(label~.,data=data.train.pure,subset=training)
time.end <- Sys.time()
rpart.duration <- time.end - time.start # Messung der Zeitdauer der Berechnung
...
plotcp(rpart.MNIST.pure); printcp(rpart.MNIST.pure)
...
randomForest.genauigkeit.pure <- matrix(0,nrow=10,ncol=10)
for(i in 1:10){
...
print(randomForest.MNIST.pure); importance(randomForest.MNIST.pure)

# Boosting - pure
set.seed(1)
time.start <- Sys.time()
boosting.MNIST.pure <- gbm(label~.,data=data.train.pure[training,],distribution="multinomial",n.trees=500,interaction.depth=32,shrinkage=0.5,n.cores=4)
time.end <- Sys.time()
boosting.duration.pure <- time.end - time.start # Messung der Zeitdauer der Berechnung
boosting.pred.pure <- predict(boosting.MNIST.pure,newdata=data.train.pure[-training,],n.trees=500) # Einordnung der Testdaten anhand des Modells
boosting.pred.pure.multi <- apply(boosting.pred.pure,1,which.max) # Zuordnung der labels
boosting.tabelle.pure <- table(pred=boosting.pred.pure.multi,true=data.train.pure[-training,1]) # Und folgende: Berechnung der Testgenauigkeit
boosting.genauigkeit.pure <- matrix(0,nrow=10,ncol=10)
for(i in 1:10){
...
bagging.prozent <- sum(diag(bagging.tabelle))/sum(bagging.tabelle)
print(bagging.MNIST); importance(bagging.MNIST)